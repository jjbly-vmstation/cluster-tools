...
check_prometheus() {
    ...
    if kubectl get svc -n "$NAMESPACE" prometheus >/dev/null 2>&1; then
        ...
        # Log Prometheus ClusterIP for debugging
        local prom_url
        ...
        if [[ -n "$prom_url" ]]; then
            ...
            # Note: Direct API query would require port-forward or cluster access
            # For now, we verify the service exists and has an IP
        fi
    else
